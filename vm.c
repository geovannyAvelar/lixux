...
pte_t *walk(uint64_t va, uint8_t alloc) {
  ...
  for (uint8_t level = 2; level > 0; level--) {
    ...
    if ((*pte & PTE_V) && ((*pte & (PTE_R|PTE_W|PTE_X)) == 0)) {
      ...
      if (*pte & PTE_V) {
        uint64_t pte_uint64 = (uint64_t)pte;
        pte_uint64 = (pte_uint64 >> 10) << 12;
        kernel_pagetable = (pagetable_t *)pte_uint64;
      } else {
        if (!alloc || (kernel_pagetable = (pagetable_t *)memset((void *)0, 0, PAGE_SIZE)) == NULL) {
          return NULL;
        }

        uint64_t new_ppn = ((uint64_t)kernel_pagetable) >> 12;
